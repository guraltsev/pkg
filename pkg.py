...
class PackageMetadata:
    ...
    def _toml_path_lines(path_entries: List[str]) -> List[str]:
        """Render ``path`` as a multi-line TOML list with one item per line."""
        if not path_entries:
            return ["path = []"]

        lines = ["path = ["]
        for entry in path_entries:
            lines.append(f"  {PackageMetadata._to_toml_scalar(entry)},")
        lines.append("]")
        return lines

    def _write_pkg_toml(self, toml_path: Path, data: Dict[str, Any], preface: Optional[List[str]] = None) -> None:
        """Write a normalized ``pkg.toml`` with ``path`` in multi-line list form."""
        lines: List[str] = []
        if preface:
