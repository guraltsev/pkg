...
Define short-lived constants next to the code that consumes them. 
Local data plus immediate use is easier to read than a top-level list whose purpose is only clear later.
Put constants at the top of the file if they impact the functionality of the module as a whole. 
...
- Inline constants that impact the module as a whole
...
def export_imporant_rows(user, order):
